...
function WalksShowController(Walk, Stop, $state, $auth) {
  ...
  function isCurrentUser() {
    return $auth.isAuthenticated() && walksShow.walk.$resolved && $auth.getPayload().id === walksShow.walk.user.id;
  }
...
WalksEditController.$inject = ['Walk', '$state', '$auth'];
function WalksEditController(Walk, $state, $auth) {
  const walksEdit = this;

  walksEdit.walk = Walk.query();

  Walk.get($state.params).$promise.then((walk) => {
    walksEdit.walk = walk;

    if(walksEdit.walk.user.id !== $auth.getPayload().id) {
      $state.go('walksIndex');
    }
